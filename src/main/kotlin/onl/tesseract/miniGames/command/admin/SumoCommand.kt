...
class SumoCommand {
    ...
    class mapsCommand{
        private val miniGame = MiniGamesPlugin.instance.miniGames[SUMO_GAMES]!!
        private val maps = miniGame.maps
        @Command(playerOnly = true)
        fun setSpawn(@Argument(value = "arena", clazz= SumoMapArguments::class) map: SumoMap,sender: Player){
            map.spawn = sender.location
        }
        @Command(playerOnly = true)
        fun listCommand(sender: Player){
            val maps = this.maps.entries.toList().joinToString { it.key+" " }
        ...
        fun createCommand(@Argument(value = "name", clazz= StringArg::class) name: String, sender: Player){
            ...
            maps[name] = SumoMap(name,sender.location);
            miniGame.saveConfig()
            sender.sendMessage(Component.text("La map $name a été créé"))
        }

        @Command
        fun deleteCommand(@Argument(value = "arena", clazz= SumoMapArguments::class) map: SumoMap, sender: Player){
            this.maps.remove(map.name)
            sender.sendMessage(Component.text("La map ${map.name} a été supprimée"))
        }

        @Command
        fun setArenaCommand(@Argument(value = "arena", clazz= SumoMapArguments::class) map: SumoMap,
        ...
        @Command
        fun resetArenaCommand(@Argument(value = "arena", clazz= SumoMapArguments::class) map: SumoMap,
                              sender: Player){
            map.resetArena()
        }

        @Command
        fun startCommand(@Argument(value = "arena", clazz= SumoMapArguments::class) map: SumoMap){
            map.start()
        }

        @Command
        fun addPlaySpawnCommand(@Argument(value = "arena", clazz= SumoMapArguments::class) map: SumoMap,
                                sender: Player){
